...
//BET PUT (UPDATE)
//Required data: token, email, optional: idMatch, bet, betId
bet.put = (req, res) => {
  // console.log(req.method)
  req.on('data', (data) => {
    let payload = Buffer.from(data).toString();
              ...
              db.read('bet', 'id', betId)
                .then(betData => {
                  ...
                  if(isBetExist) {
                    ...
                    db.update('bet', dataBet, 'id', betId)
                      .then(() => {
                        helpers.response(res, 200, {
                      ...
                      .catch((err) => {
                        helpers.response(res, 500, {
                          'Error': 'Could not update bet'
                        });
                      });
                  } else {
                    helpers.response(res, 403, {
                      'Error': 'Could not update, bet is not exist'
                    });
                  }
                }).catch((err) => {
                    helpers.response(res, 500, {
  ...
  if(email && idBet) {
    ...
    helpers.isAdminVerify(email, token, req, res).then(
      result => {
        ...
        db.read('bet', 'id', idBet)
          .then((existBet) => {
            console.log(userId, existBet[0].id_user);
            if(userId === existBet[0].id_user) {
            db.delete('bet', 'id', idBet)
              .then(() => {
                helpers.response(res, 200, {
                  'Message': 'Bet has been deleted'
                });
              }).catch(err => console.log("Can't delete " + err));
            } else {
              helpers.response(res, 200, {
                'Message': 'Bet is not for this user'
              });
            }
          }).catch(err => {
            helpers.response(res, 200, {
              'Message': 'Bet is not exist'
            });
          });
