...
helpers.isAdminVerify = (email, token, req, res) => { return new Promise((resolve, reject) => {
  if (email && token) {
...
helpers.sendEmail = (userEmail, key) => {
  nodemailer.createTestAccount((err, account) => {
    ...
    let mailOptions = {
      ...
      html: `<h1>Betting</h1><p><a href="http://localhost:` + config.port + `/register/${key}">Aktywuj</a></p>` // html body
    };
