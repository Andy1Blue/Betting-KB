          ...
          if(data.message) {
            get('register', 'user_ip', user.ip).then((data) => {
              if(Array.isArray(data) && date - data[0].date < 60000){
                ...
                rej({message:`You must wait a moment`});
                con.end();
              }else {
                if(Array.isArray(data) && data[0].id) {
                  deleteId('register', data[0].id)
                }
                add(obj).then(data => console.log(data)).catch((err) =>console.log(err))
                con.query(`INSERT INTO register (user_name, user_email, user_password, user_ip, user_key, date) VALUES ("${user.name}", "${user.email}", "${user.password}","${user.ip}", "${key}", ${date})`, err => {
                  if (err) {
                    rej(err);
                    con.end();
                  } else {
            ...
            rej({message:`User ${user.email} already exist`});
            con.end();
          }
        })        
      }
...
add = (user) => {
  return new Promise((res,rej) => {
    // let keys = Object.keys(objToDB)
    // let fields = keys.join(',');
    // let values = Object.values(objToDB);
    // let x = values.join(',')
    let con = mysql.createConnection(config.db);
    con.connect(err => {
      if(err) {
        ...
        con.query(`INSERT INTO user (user_name, user_email) VALUES ("${user.name}", "${user.email}")`, err => {
          if(err) {
            rej(err)
            con.end()
          }else {
      ...
      if (err) {
        ...
        con.query(`SELECT * FROM ${tab} WHERE UPPER(${tabField}) = "${query.trim().toUpperCase()}"`, (err, result) => {
          if (err) {
            ...
            res({message:`User ${query} does not exist`, isExist:false});
            con.end();
