  ...
  return new Promise((res, rej) => {
    get('register', 'user_key', key).then(user => {
      if(Array.isArray(user) && user[0].user_email) {
        ...
        res({message:'User has been added'})
      }else {
        rej({message:'Key does not exist'})

      }
    }).catch(err => rej({test:'sssss'}))
  ...
  return new Promise((res,rej) => {
    ...
    con.connect(err => {
      if(err) {
        ...
        con.query(`INSERT INTO user (user_name, user_email) VALUES ("${user.user_name}", "${user.user_email}")`, err => {
          if(err) {
            rej(err)
            con.end()
          }else {
